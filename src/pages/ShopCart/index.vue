  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="good in cartInfoList" :key="good.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="delCartById(good)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked&&cartInfoList.length" @change="updateAllChecked" />
        <span>全选</span>
      </div>
      <div class="option">
...
import throttle from "lodash/throttle";
  export default {
    ...
    computed:{
      ...
      // 判断是否全选
      isAllChecked(){
        // 判断cartInfoList中每个元素的isChecked是否为1，全为1返回true，否则返回false
    ...
    methods: {
      ...
      handler:throttle(async function(type,disNum,good){
        ...
        // 派发action
        try {
          await this.$store.dispatch('detail/addOrUpdateShorCart',{skuId:good.skuId,skuNum:disNum})
          // 修改成功后，重新获取数据
          this.getData()
        } catch (error) {
          alert(error.message)
      ...
      async delCartById(cart){
        try {
            await this.$store.dispatch('shopcart/deleteCartListById',cart.skuId)
            this.getData()
          } catch (error) {
            alert(error.message)
          }
