    ...
    methods: {
      ...
      async open() {
        ...
        this.$alert(`<img src=${url} />`, '微信支付', {
          ...
          cancelButtonText:'支付遇到问题了', // 取消按钮的文本内容
          confirmButtonText:'已支付成功', //确定按钮的文本内容
          showClose:false, // MessageBox 是否显示右上角关闭按钮
          beforeClose:(type,instance,done)=>{
            // type：区分是确定还是取消按钮 instance：当前组件实例  done：关闭弹出框的方法
            if(type==='cancel'){
              alert('请联系管理员')
              // 清除定时器
        ...
        // 弹出二维码之后，我们需要知道用户是否支付成功
        // 支付成功则进行路由跳转，支付失败则提示信息
        // 开启定时器，隔一段时间向服务器发送请求，询问用户的支付状态
        if(!this.timer){
          this.timer=setInterval(async ()=>{
            ...
            if(result.code===200){
              ...
              clearInterval(this.timer)
              this.timer=null
              this.code=result.code // 保存支付成功的code
