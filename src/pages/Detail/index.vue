      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodNum" @change="changegoodNum"/>
...
export default {
  ...
  methods: {
    changeActive(saleAttrVlaue,arr){
      arr.forEach(item => {
        item.isChecked=item===saleAttrVlaue?'1':'0'
      });
    },
    changegoodNum(event){
      let input=event.target.value
      if(parseInt(input) && parseInt(input)>=1){
        this.goodNum=parseInt(input)
      }else{
        this.goodNum=1
      }
    },
    async addShopcart(){
      ...
      // 3.失败，给用户进行提示
      try {
        await this.$store.dispatch('detail/addOrUpdateShorCart',{skuId:this.$route.params.goodId,skuNum:this.goodNum})
  ...
  mounted() {
    // 获取产品详情信息
    this.$store.dispatch("detail/getGoodsInfo", this.$route.params.goodId);
  },
};
</script>
